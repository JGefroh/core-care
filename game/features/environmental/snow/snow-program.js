...
import { default as fragmentShaderSourceCode } from './snow-fragment-shader';
import { default as vertexShaderSourceCode } from './snow-vertex-shader';
...
class Program extends BaseProgram {
  ...
  uploadInstanceData(commands) {
      for (let command of commands) {
        ...
        this.instanceBuffers.accumulations.push(command.accumulation)
      }
    }
    ...
    _flushInstanceDataBuffer(renderCtx, perFrameCache) {
      ...
      this._bindToBufferIfExists(renderCtx, this.getBuffers(), `INSTANCE_ACCUMULATION_${index}`, this.instanceBuffers.accumulations)
    ...
    _initializeBuffers(renderCtx) {
        ...
        for (let index = 0; index < this.flushCountMax; index++) {
          ...
          this.initializeBuffersFor(renderCtx, `INSTANCE_ACCUMULATION`, this.maxBufferSize, renderCtx.DYNAMIC_DRAW, 'float32', index, () => {
            const locAccumulation = renderCtx.getAttribLocation(program.program, 'a_instanceAccumulation');
            renderCtx.enableVertexAttribArray(locAccumulation);
            renderCtx.vertexAttribPointer(locAccumulation, 1, renderCtx.FLOAT, false, 0, 0);
            renderCtx.vertexAttribDivisor(locAccumulation, 1); // per-instance
          })
